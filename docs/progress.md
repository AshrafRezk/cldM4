...
**Current phase:** A (Ollama live; worker **not** on `:8080` — warmup blocked listen + httpx `%d` log redact; pull + reinstall)  
...
- [ ] Worker on **`127.0.0.1:8080`**. zsh `.env` glob is fixed (bash loader). Next failure (2026-09-19 smoke): unit tests **56 passed**, live still down. `worker.err` shows lifespan stuck in `await ollama.warmup()` / httpx `POST /api/generate` 200 with `Message: 'HTTP Request: ... %d ...'` (redact filter broke `%d`; uvicorn does not bind until warmup `yield`s; qwen3.5 thinking + default stream can hang generate). Fix in git: background warmup, `stream: false`/`think: false`, format-then-redact.
