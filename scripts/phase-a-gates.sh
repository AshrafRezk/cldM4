...
if command -v ollama >/dev/null 2>&1; then
  ...
  if ! ollama show "$DEFAULT_MODEL" >/dev/null 2>&1 && [ "$DO_PULL" = "1" ]; then
    info "pulling $DEFAULT_MODEL (Gemma E4B QAT is ~6.1 GB; Wi-Fi makes it slower)"
  ...
  if SHOW="$(ollama show "$DEFAULT_MODEL" 2>/dev/null)"; then
    ...
    info "Walk the ladder in PLAN.md §7. Take the first that resolves, supports tools AND vision, and fits ~4-8 GB:"
    info "  1. gemma4:e4b-it-qat (intended QAT Q4_0)"
    info "  2. gemma4:12b-it-qat"
    info "  3. current E4B QAT-equivalent on https://ollama.com/library/gemma4/tags — not gemma4:latest unless show proves Q4_0 ~6 GB"
    info "  4. qwen3.5:9b only if every Gemma vision tag fails (Arabic/vision degraded — STOP and tell the operator)"
    info "  5. llama3.2:3b last resort"
    info "Then set DEFAULT_MODEL in ~/Cloudiator/.env, record it in docs/operator-checklist.md §8,"
    info "and change nothing else. Do not substitute 26B, 31B, Q8, or bf16."
